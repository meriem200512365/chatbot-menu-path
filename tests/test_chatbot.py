...
# dans tests/test_chatbot.py
def test_recherche_utilise_question_brute_pas_etendue():
    """Regression: l'expansion de synonymes degradait la recherche
    semantique avec le modele fine-tune (RO -> '(Regime obligatoire)'
    faisait sortir le bon resultat de la zone fiable)."""
    from src.chatbot.chatbot import repondre
    rep = repondre("je veux gerer les actes RO")
    assert rep["type"] == "reponse_directe"
    assert "Gestion des Actes RO" in rep["resultats"][0]["path_str"]
