...
Le seuil de pertinence est importe depuis src/config.py -- IDENTIQUE a
celui utilise par src/chatbot/chatbot.py, pour que les deux modes de
reponse (direct / RAG) soient d'accord sur ce qui est "fiable" ou non.
"""

from src.config import RAG_TOP_K, SEUIL_DISTANCE_FIABLE
from src.search.semantic_search import search
from src.search.synonyms import etendre_question
...
def repondre_rag(question: str, top_k: int = RAG_TOP_K, document_context: str = None) -> dict:
    ...
    question_etendue = etendre_question(question)
    resultats = search(question, top_k=top_k)
    # Meme seuil (en distance) que le mode direct -> les deux modes sont
    # coherents sur ce qu'ils considerent comme un match fiable.
    pertinent = bool(resultats) and resultats[0]["distance"] < SEUIL_DISTANCE_FIABLE
    ...
    blocs.append(f"Question de l'utilisateur : {question_etendue}")
    blocs.append("Reponds a la question.")
    user_prompt = "\n\n".join(blocs)

    # --- Etape 3 : Generation ---
    reponse_llm, usage = generate(SYSTEM_PROMPT, user_prompt)
