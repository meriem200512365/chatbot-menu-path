...
def analyser_image(image_bytes: bytes, mime_type: str, question: str) -> tuple:
    """
    Envoie l'image + la question au modele vision.
    Retourne un tuple (texte_genere, usage) -- voir llm_client.generate()
    pour le format de `usage`.
    """
    if not GROQ_API_KEY:
...
if __name__ == "__main__":
    with open("exemple.png", "rb") as f:
        img_bytes = f.read()
    reponse, usage = analyser_image(img_bytes, "image/png", "Que montre cette image ?")
    print(reponse)
    print(usage)